...
// Rich 3-repetition kitchen order notification chime (3.5 seconds) using Web Audio API
export function playOrderChime() {
  ...
  try {
    ...
    const playSingleDingDong = (startTime: number) => {
      const now = startTime;

      // Tone 1: Warm resonant chime (D5 -> A5)
      ...
      // Tone 2: Crisp harmonic sparkle (D6 -> E6)
      const osc2 = ctx.createOscillator();
      const gain2 = ctx.createGain();
      osc2.type = "triangle";
      osc2.frequency.setValueAtTime(1174.66, now + 0.12);
      osc2.frequency.exponentialRampToValueAtTime(1318.51, now + 0.24);
    ...
    // 3'lü zil tekrarı: 0.0s, 1.1s, 2.2s (Toplam ~3.5 saniye)
    playSingleDingDong(baseNow);
    playSingleDingDong(baseNow + 1.1);
    playSingleDingDong(baseNow + 2.2);
  } catch (e) {
